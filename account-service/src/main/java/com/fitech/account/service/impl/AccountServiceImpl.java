...
//                    	try{
//                    		 dicItemName =dictionaryItemRepository.findById(Long.parseLong(accountField1.getValue().toString())).getDicItemName();
...
//            }
            ...
            Collection<AccountLine> accountLineCol = 
            		accountProcessVo.getAccount().getAccountLine().getContent();
...
//          AccountTemplate accountTemplate = accountTemplateRepository.findOne(templateId);
            ...
            ExcelUtils.createExcel(hList, sheetName, CommonConst.getProperties("template_path"),
                    downRows,downData);
            return sheetName;
        }
        ...
        try {
            ...
            if (isExcel) {
                ...
                if (account != null) {
                    ...
                    String validateTableName = account.getAccountTemplate().getBusSystem().getReportSubSystem().getSubKey() + ":" + account.getAccountTemplate().getTableName();
                    Collection<ValidateAnalyzeResult> rules = validateAnalyzeResultService.findByObjectID(validateTableName, operateFieldArr);
...
//    @Override
//    public GenericResult<Boolean> addAccountData(AccountProcessVo accountProcessVo) {
//        GenericResult<Boolean> result = new GenericResult<>();
//        try {
//            if(accountProcessVo == null || accountProcessVo.getAccount() == null ||
...
//    @Override
//    public GenericResult<Boolean> modifyAccountData(AccountProcessVo accountProcessVo) {
//        GenericResult<Boolean> result = new GenericResult<>();
//        try {
//            if (accountProcessVo == null || accountProcessVo.getAccount() == null ||
//                    accountProcessVo.getAccount().getId() == null) {
...
//            accountEditLogService.saveAccoutnEditLogItem(accountEditLog, accountLines.get(0).getAccountFields(), account);
//        } catch (Exception e) {
//            e.printStackTrace();
//            throw new AppException(ExceptionCode.SYSTEM_ERROR, e.toString());
//        }
...
//    }
    ...
    public GenericResult<Object> validateAll(AccountProcessVo accountProcessVo) {
        ...
        if (objectValidateRuleService == null) {
            ...
            Collection<ValidateAnalyzeResult> rules = validateAnalyzeResultService.findByObjectID(validateTableName, accountProcessVo.getOperateFieldArr());
            if(!rules.isEmpty()){
    ...
    public GenericResult<Object> validatePL(List<Long> idList, Long userId) {
        ...
        for (Long id : idList) {
            //根据taskid查找accountid
            String sql = "SELECT ACC.ID FROM ACCOUNT ACC  inner JOIN AccountProcess ACCPRO ON ACC.id=ACCPRO.ACCOUNT_ID inner JOIN ACT_RU_TASK task on task.proc_inst_id_=ACCPRO.PROCINSETID where  TASK.ID_=" + id;
            long accountId = 0;
            ...
            Collection<AccountField> accountFieldCollection = accountProcessVoGenericResult.getData().getAccount().getAccountTemplate().getAccountFields();
            ...
            if (objectValidateRuleService == null) {
                ...
                Collection<ValidateAnalyzeResult> rules = validateAnalyzeResultService.findByObjectID(validateTableName, operateFieldStr);  //校验公式
                if(!rules.isEmpty()){
                	ValidateBatch validateBatch = validateAnalyzeResultService.excuteFormu(rules, accountId + "");
                    Collection<ValidateResult> list = validateResultService.findByValidatebatch(validateBatch.getBatchId());  //校验结果
                    if (list.size() > 0) {
                    	result.setSuccess(false);
                        result.setMessage("校验不通过");
...
//    @Override
//    @Transactional
//    public GenericResult<Boolean> batchUpdateAccounData(AccountProcessVo accountProcessVo) {
...
//    }
    ...
    public List<String> batchUpdateAccounData(AccountProcessVo accountProcessVo) {
        ...
        try {
            //根据id找到数据库中account
            Account account = accountRepository.findOne(accountProcessVo.getAccount().getId());
            Collection<AccountField> accountFieldList = accountProcessVo.getAccount().getAccountLines().get(0).getAccountFields();
            //业务条线：表名
            String validateTableName = account.getAccountTemplate().getBusSystem().getReportSubSystem().getSubKey() + ":" + account.getAccountTemplate().getTableName();
